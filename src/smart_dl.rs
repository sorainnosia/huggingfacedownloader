...
pub async fn smart_download(
	...
	let handle = tokio::spawn(async move {
		...
		let result = if range_ok && total_size.is_some() {
			download_in_chunks(context.clone(), &client2.clone(), url2.as_str(), filename2.as_str(), total_size.unwrap(), parallel_count, chunk_count, max_size2, cancel_notify, multi.clone(), resumable).await
		} else {
			download_whole_with_progress(context.clone(), &client2.clone(), url2.as_str(), filename2.as_str(), cancel_notify.clone(), multi.clone(), resumable).await
		};
...
pub fn slice_chunk(total_size: u64, chunk_count: usize, max_size: Option<u64>, start2: u64) -> Vec<(u64, u64, u64)> {
	let mut result = vec![];
	...
	for i in 0..chunk_count {
		...
		if let Some(m) = &max_size {
			if end + 1 > start {
				if length > *m {
					let temps = slice_chunk(length, chunk_count, max_size.clone(), start + start2);
					result.extend(temps);
				} else {
...
pub async fn download_in_chunks(
	...
	let mut slices = vec![];
	if max_size.is_none() {
		slices = slice_chunk(total_size, chunk_count, max_size, 0);
	} else {
